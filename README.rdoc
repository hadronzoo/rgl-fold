...
The RGLFold module adds the fold and compile_fold methods to RGL[http://rgl.rubyforge.org/rgl/] graphs. Fold iterates over all paths accessible in a graph from a given vertex, combining successive vertices using a given block. These methods work on both cyclic and acyclic graphs.
